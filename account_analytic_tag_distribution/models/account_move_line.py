...
class AccountMoveLine(models.Model):
    ...
    def _onchange_analytic_tag_ids(self):
        ...
        distribution."""
        if any(
            self.analytic_tag_ids.mapped("active_analytic_distribution")
        ):
            self.analytic_distribution = False

    @feature(
        fallback=lambda self: super(AccountMoveLine, self)._prepare_analytic_lines(),
    )
    def _prepare_analytic_lines(self):
        ...
        tags_with_distribution = self.analytic_tag_ids.filtered(
            lambda x: x.active_analytic_distribution and x.analytic_distribution
        )
